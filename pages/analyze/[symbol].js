    ...
    <section className="rounded-2xl border border-white/10 bg-[#141b2d] p-5">
      ...
      <ul className="space-y-2">
        {list.map((s, i) => (
          <li
            key={i}
            className="flex justify-between border border-white/10 bg-[#0b1220] rounded-lg px-3 py-2"
          >
...
function AIGalaxyMap() {
  return (
    <section className="rounded-2xl border border-white/10 bg-[#141b2d] p-6 mt-6">
      <h2 className="text-lg font-semibold text-center text-emerald-400 mb-4">
        🌌 AI Galaxy Trend Map
      </h2>
      <div className="grid grid-cols-10 gap-[3px]">
        {Array.from({ length: 100 }).map((_, i) => {
          const c =
            i % 3 === 0 ? "#22c55e" : i % 3 === 1 ? "#facc15" : "#ef4444";
          return (
            <div
              key={i}
