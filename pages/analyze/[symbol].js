// ✅ /pages/analyze/[symbol].js — Visionary Analyzer (Stock + Option + Price Check)
...
export default function Analyze() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [inf, opt, coreRes, scan, newsRes] = await Promise.all([
        ...
        const base = inf && !inf.error ? inf : coreRes;
        setCore(base);
        setScanner(scan);
        setOptionAI(opt);
  ...
  const hist = core?.chart?.timestamps?.map((t,i)=>({
    time:t,open:core.chart.open?.[i],high:core.chart.high?.[i],
    low:core.chart.low?.[i],close:core.chart.prices?.[i],volume:core.chart.volume?.[i]
  })) || [];
  ...
  return (
    <main className="min-h-screen bg-[#0b1220] text-white text-[13px] font-semibold">
      <div className="max-w-6xl mx-auto px-3 py-5 space-y-5">
        ...
        {mode==="option" && <OptionPriceCheck optionAI={optionAI}/>}
...
/* ✅ Option Price Check */
function OptionPriceCheck({ optionAI }) {
  const [delta,setDelta]=useState(0.2);
  const [theta,setTheta]=useState(-0.008);
  const [stockMove,setStockMove]=useState(1);
  const [days,setDays]=useState(2);
  const [premium,setPremium]=useState(optionAI?.topCall?.premium||0.4);
  const [result,setResult]=useState(null);

  const checkPrice=()=>{
    const newPrice=premium+delta*stockMove+theta*days;
    const diff=((newPrice-premium)/premium)*100;
    setResult({newPrice,diff});
  };

  return (
    <section className="bg-[#0f172a] rounded-2xl border border-pink-500/20 p-3 space-y-2">
      <h3 className="text-pink-400 font-bold text-[12px]">Option Price Check 💹</h3>
      <div className="grid grid-cols-2 gap-1.5 text-[12px]">
      ...
      <div className="grid grid-cols-3 gap-1.5 text-[12px]">
        <input type="number" step="0.1" value={stockMove} onChange={e=>setStockMove(+e.target.value)} className="bg-[#1b2435] text-center p-1 rounded border border-white/10" placeholder="หุ้นขึ้น ($)"/>
        <input type="number" step="1" value={days} onChange={e=>setDays(+e.target.value)} className="bg-[#1b2435] text-center p-1 rounded border border-white/10" placeholder="วัน"/>
        <input type="number" step="0.01" value={premium} onChange={e=>setPremium(+e.target.value)} className="bg-[#1b2435] text-center p-1 rounded border border-white/10" placeholder="ราคา Option"/>
      </div>
      <button onClick={checkPrice} className="w-full py-1.5 bg-emerald-500/20 border border-emerald-400/30 rounded text-emerald-400 font-semibold text-[12px] hover:bg-emerald-500/30">คำนวณราคา</button>
      {result && (
        <div className={`text-center text-[12px] font-bold mt-1 ${result.diff>0?"text-emerald-400":result.diff<0?"text-red-400":"text-yellow-300"}`}>
          ควรได้ราคา ≈ ${result.newPrice.toFixed(2)} ({result.diff>0?"+":""}{result.diff.toFixed(1)}%)
        </div>
      )}
    </section>
  );
