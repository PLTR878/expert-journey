...
/* =========================
   🛰️ AUTO SCAN ทั้งตลาด (AI) — Real-time Version
========================= */
function AutoMarketScan() {
  ...
  const runScan = () => {
    ...
    es.onmessage = (e) => {
      ...
      try {
        ...
        if (data.hit) {
          ...
          setMessages((p) => [...p, { id: Date.now(), msg: data.hit.symbol }]);
          beep();
          vibrate();
        }
    ...
    es.onerror = () => {
      setLogs((prev) => [...prev, "❌ การเชื่อมต่อ EventSource ขัดข้อง"]);
      es.close();
    };
  };
