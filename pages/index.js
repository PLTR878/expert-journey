...
const DEFAULT_UNIVERSE = []; // ว่างไว้ก่อน
...
export default function Home() {
  ...
  async function run() {
    ...
    try {
      ...
      // ถ้าไม่มี API จริง ให้ mock data
              ...
              return (
                ...
                >
                  ...
                  <td className="p-2 text-right">
                    {price}{' '}
                    <small
                      className={(q?.changePct || 0) >= 0 ? 'text-green-400' : 'text-red-400'}
                    >
                      {change}
                    </small>
                  </td>
                  ...
                  <td className="p-2 text-center">
                    {[r.e20, r.e50, r.e200].map(x => (x ? x.toFixed(2) : '-')).join(' / ')}
                  </td>
                  <td className="p-2 text-center">
                    <a
