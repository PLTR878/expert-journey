...
</div>  );  const favoriteData = favorites.map((s) => ({
symbol: s,
...favoritePrices[s],
}));
...
</div>    </header>    {/* เนื้อหาแต่ละแท็บ */}    <div className="max-w-6xl mx-auto px-4 py-4">      
    {/* Favorites */}      
    {activeTab === "favorites" && (      
      <>      
...
)}    </div>    {/* Bottom Navigation */}    <nav className="fixed bottom-0 left-0 right-0 bg-[#0e1628]/90 border-t border-white/10 backdrop-blur flex justify-around text-gray-400 text-[12px] z-50">      
    <button      
