...
export default function Home() {
  ...
  async function loadDiscovery(retry = 0) {
    try {
      ...
      addLog("🌋 AI Discovery Pro กำลังโหลดพอร์ตหุ้นต้นน้ำล่าสุด...");

      // ✅ ดึงข้อมูลจากพอร์ต AI ที่ cron-job อัปเดตทุก 2 นาที
      const res = await fetch("/api/ai-portfolio?_t=" + Date.now(), {
        cache: "no-store",
      });

      if (!res.ok) throw new Error(`API ${res.status}`);
      const j = await res.json();
      const list = j.top30 || j.discovered || [];

      if (!list.length) throw new Error("ไม่พบข้อมูลหุ้นจากพอร์ต AI");

      const formatted = list.map((r) => ({
        symbol: r.symbol,
        lastClose: parseFloat(r.price) || 0,
        rsi: r.rsi || 0,
        reason: r.reason || "AI วิเคราะห์ศักยภาพต้นน้ำ",
        aiScore: r.aiScore || 0,
        trend:
          r.signal === "Buy"
            ? "Uptrend"
      ...
      addLog(`✅ โหลดหุ้นจากพอร์ต AI สำเร็จ ${formatted.length} ตัว`);
