...
export const config = {
  runtime: "edge", // ✅ ทำงานแบบ Edge Function (เร็วและไม่ timeout บน Vercel)
};
...
export default async function handler(req) {
  try {
    ...
    const newsSentiment = async (sym) => {
      try {
        ...
        for (const n of items) {
          ...
          if (/(ai|growth|record|upgrade|expand|beat|contract|partnership)/.test(t)) score += 2;
    ...
    for (const sym of sample) {
      ...
      await new Promise((r) => setTimeout(r, 40)); // กัน timeout
    }
