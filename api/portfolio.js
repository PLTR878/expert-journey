...
export default async function handler(req, res) {
  try {
    ...
    const response = await fetch(`https://rt-ai-peach.vercel.app/api/proxy?symbol=${query}`);
    ...
    const portfolio = holdings.map(h => {
      ...
      return { symbol: h.symbol, hold: h.hold, price, value: h.hold * price };
    });

    const total = portfolio.reduce((sum, x) => sum + x.value, 0);
    const profit = (total - capital).toFixed(2);

    res.status(200).json({ capital, portfolio, total, profit });
  } catch (err) {
